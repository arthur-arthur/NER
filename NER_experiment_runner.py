...
#########################
DATA = 'conll2002'
LANG = 'multi'                         # !important: determines the correct transformer and bytepair embedding
...
class NER_experiment():
    ...
    def build_embedding(self, lang, embedding_codes: List[str]) -> None:
        ...
        for code in embedding_codes:
            ...
            code = code.lower()
            assert (
                code in ["bpe", "bert", "flair", "ft", "char", "ohe"]
    ...
    def _extract_from_log(self) -> Dict[str, int]:
        ...
        for entity_class in log:
            ...
            weights.append(np.sum(results[:4]))     # tp, fp, fn, tn  -->  count instances/class
